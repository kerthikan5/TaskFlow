...
export function DashboardPage() {
  ...
  const pendingTasks = tasks.filter(t => t.status === TaskStatus.ToDo || t.status === TaskStatus.InProgress).length;
              ...
              <div className="list">
                {projects.slice(0, 5).map(p => (
                  <Link to={`/projects/${p.id}`} key={p.id} className="list-item">
                    ...
                    <span className={`badge status-${p.status}`}>{['Active','On Hold','Done','Archived'][p.status]}</span>
                  </Link>
                ))}
                {projects.length === 0 && <p className="empty-state">No projects yet. <Link to="/projects">Create one →</Link></p>}
              </div>
            </div>
                ...
                {tasks.map(t => (
                  <div key={t.id} className="list-item">
                    ...
                    <div className="badge-group">
                      <span className={`badge ${priorityColors[t.priority]}`}>{TaskPriorityLabel[t.priority]}</span>
                      <span className={`badge ${statusColors[t.status]}`}>{TaskStatusLabel[t.status]}</span>
                    </div>
                  </div>
                ))}
                {tasks.length === 0 && <p className="empty-state">No tasks assigned to you.</p>}
              </div>
            </div>
          </div>
        </>
